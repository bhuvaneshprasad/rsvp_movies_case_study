...
/* The data sets are imported into mysql, let’s explore some of the tables. 
...
SELECT
		SUM(CASE WHEN id IS NULL THEN 1 ELSE 0 END) AS null_count_in_id,
		SUM(CASE WHEN title IS NULL THEN 1 ELSE 0 END) AS null_count_in_title,
		SUM(CASE WHEN year IS NULL THEN 1 ELSE 0 END) AS null_count_in_year,
		SUM(CASE WHEN date_published IS NULL THEN 1 ELSE 0 END) AS null_count_in_date_published,
		SUM(CASE WHEN duration IS NULL THEN 1 ELSE 0 END) AS null_count_in_duration,
		SUM(CASE WHEN country IS NULL THEN 1 ELSE 0 END) AS null_count_in_country,
		SUM(CASE WHEN worlwide_gross_income IS NULL THEN 1 ELSE 0 END) AS null_count_in_worlwide_gross_income,
		SUM(CASE WHEN languages IS NULL THEN 1 ELSE 0 END) AS null_count_in_languages,
		SUM(CASE WHEN production_company IS NULL THEN 1 ELSE 0 END) AS null_count_in_production_company
FROM movie;

-- Now as we can see four columns of the movie table has null values. Let's look at the movies released each year. 
...
FROM movie
WHERE
	(country LIKE '%India%' OR country LIKE '%USA%') AND
    (year = 2019);
...
LEFT JOIN genre g ON m.id = g.movie_id
LEFT JOIN ratings r ON m.id = r.movie_id
WHERE MONTH(date_published) = 03 AND year = 2017 AND m.country LIKE '%USA%' AND r.total_votes > 1000
GROUP BY g.genre;
...
FROM movie m
LEFT JOIN ratings r ON m.id = r.movie_id
WHERE date_published >= '2018-04-01' AND date_published <= '2019-04-01' AND r.median_rating = 8;
...
SELECT
	...
	ELSE
		'No'
	END AS German_has_more_votes_then_Italian;
...
JOIN genre g ON m.id = g.movie_id
JOIN ratings r ON m.id = r.movie_id
WHERE avg_rating > 8 AND genre IN (SELECT genre FROM top_three_genres)
...
WITH top_actresses AS
		...
		JOIN movie m ON rm.movie_id = m.id
		JOIN ratings r ON m.id = r.movie_id
		WHERE category = 'actress' AND m.country Like '%India%' AND m.languages LIKE '%Hindi%'
		GROUP BY n.name
...
SELECT
    ...
    ROUND(AVG(AVG(duration)) OVER(ORDER BY genre), 2) As moving_average
FROM genre g
...
WITH diff_date_between_two_movies AS (
							SELECT
								dm.name_id,
								NAME,
								dm.movie_id,
	...
	top_director_summary AS (
							...
							FROM diff_date_between_two_movies )
SELECT
